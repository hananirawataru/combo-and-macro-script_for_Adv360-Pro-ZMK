...
# for key_combo, letter in zip(key_combos, symbols):
#         combo_name = symbol.replace('(','').replace(')','')
...
# symbols = ['QMARK', 'LA(SLASH)', 'TILDE', 'LBKT', 'RBKT',\
#  'LA(LBKT)', 'LA(RBKT)', 'LS(N9)', 'LS(N0)','LS(LBKT)', 'LS(RBKT)',\
#  'MINUS', 'LA(BACKSLASH)']

# def write_symbol_macro(symbols):
#     for symbol in symbols:
#         macro_name = symbol.replace('(','').replace(')','')
#         f.write('macro_' + macro_name + ': macro_' + macro_name + '{\n')
#         f.write('compatible = "zmk,behavior-macro";\n')
#         f.write('label = "macro_' + macro_name + '";\n')
#         f.write('#binding-cells = <0>;\n')
#         f.write('bindings = <&kp ' + symbol + '>;\n')
#         f.write('};\n\n')

# path_w = 'macro.dtsi'
# with open(path_w, mode='w') as f:
#     write_change_lang_macro()
#     write_symbol_macro(symbols)
#     write_letter_macro(non_combo_letters)
#     write_letter_macro(combo_letters)

# def write_symbol_combos(symbol_fingers, thumbs, symbols):
#     key_combos = [(finger, thumb) for thumb in thumbs for finger in symbol_fingers]
#     double_symbols = symbols + symbols
#     for key_combo, letter in zip(key_combos, symbols):
#         combo_name = symbol.replace('(','').replace(')','')
#         f.write('\tcombo_' + letter + ' {\n')
#         f.write('\t\ttimeout-ms = <' + str(timeout_ms) + '>;\n')
...
# def write_letter_combos(letter_fingers, thumbs, combo_letters):
#     key_combos = [(finger, thumb) for thumb in thumbs for finger in letter_fingers]
...
#     f.write('combos {\n')
#     f.write('\tcompatible = "zmk,combos";\n\n')
#     write_letter_combos(letter_fingers, thumbs, combo_letters) 
#     f.write('};')
